      ...
      <div class="card page-content">
        <div class="blog-container">
              ...
              <ul class="nav">
                <li class="nav-item" v-for="(ft,i) in filters" :key="i">
                  <div
                    ...
                    @click="filter=ft.key"
                    class="nav-link cursor-pointer">{{ft.name}}
                  </div>
                </li>
              </ul>
              <ul class="nav">
          ...
          <div class="post-list-wrapper">
            <post-item @click.native="view(post)" v-for="post in posts" :post="post" :key="post._id"></post-item>
...
<script>
  ...
  import {Http, Codes, QueryString} from '../plugins/utils/api'
  ...
  export default {
    ...
    methods: {
      async loadMoreHandle($state) {
        this.isLoading = true;
        this.page.p++;
        let posts = await Http.get('posts?' + QueryString.stringify({
          filter: this.filter,
          ...this.page
        }));
        $state.loaded();
